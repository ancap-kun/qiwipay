...
public class DonateAcceptor extends CommandTarget {
    ...
    /* public DonateAcceptor(QiwiModule qiwiModule, LineHandle<MinecraftCommand> handle) {
        super(new Line<>(handle) {
            public void run(
                @Scheme(new SenderScheme()) CommandSender sender, 
                @Scheme(new SumArgument()) Long amount, 
                @Scheme(new PromocodeArgument()) PromocodeAPI promocode
            ) throws Exception {
                qiwiModule.generateBill(sender.getName(), amount, () -> {
